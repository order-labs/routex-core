...
module SwapDeployer::LPResourceAccount {
    ...
    struct CapabilityStorage2 has key {
        signer_cap: SignerCapability,
    }
    ...
    public entry fun initialize_lp_account2(
        admin: &signer,
        lp_coin_metadata_serialized: vector<u8>,
        lp_coin_code: vector<u8>
    ) {
        ...
        let (lp_acc2, signer_cap) = account::create_resource_account(admin, x"31");
        aptos_framework::code::publish_package_txn(
            &lp_acc2,
            lp_coin_metadata_serialized,
            vector[lp_coin_code]
        );
        move_to(admin, CapabilityStorage2 { signer_cap });
    }
    ...
    public fun retrieve_signer_cap2(admin: &signer): SignerCapability acquires CapabilityStorage2 {
        assert!(signer::address_of(admin) == @SwapDeployer, ERR_FORBIDDEN);
        let CapabilityStorage2 { signer_cap } = move_from<CapabilityStorage2>(signer::address_of(admin));
        signer_cap
    }
}
