module SwapDeployer::AnimeSwapPoolV2 {
    use ResourceAccountDeployer2::LPCoinV2::LPCoin2;
    ...
    struct LiquidityPool<phantom X, phantom Y> has key {
        ...
        k_last: u128,
        lp_mint_cap: MintCapability<LPCoin2<X, Y>>,
        lp_freeze_cap: FreezeCapability<LPCoin2<X, Y>>,
        lp_burn_cap: BurnCapability<LPCoin2<X, Y>>,
        locked: bool,
    }
    ...
    const RESOURCE_ACCOUNT_ADDRESS: address = @ResourceAccountDeployer2;

    // initialize
    fun init_module(admin: &signer) {
        // init admin data
        let signer_cap = LPResourceAccount::retrieve_signer_cap2(admin);
    ...
    public fun get_pair_meta<X, Y>(): PairMeta {
        ...
        let lp_coin_type_info = type_info::type_of<LPCoin2<X, Y>>();
        PairMeta {
    ...
    fun mint_fee_interval<X, Y>(
        ...
        if (fee_on) {
            if (k_last != 0) {
                ...
                let total_supply = AnimeSwapPoolV1Library::get_lpcoin_total_supply<LPCoin2<X, Y>>();
    ...
    fun mint_coin<X, Y>(
        account: &signer,
        amount: u64,
        mint_cap: &MintCapability<LPCoin2<X, Y>>
    ) {
        let acc_addr = signer::address_of(account);
        if (!coin::is_account_registered<LPCoin2<X, Y>>(acc_addr)) {
            coin::register<LPCoin2<X, Y>>(account);
        };
        let coins = coin::mint<LPCoin2<X, Y>>(amount, mint_cap);
    ...
    public entry fun remove_liquidity_entry<X, Y>(
        ...
        let (x_out, y_out);
        if (AnimeSwapPoolV1Library::compare<X, Y>()) {
            let coins = coin::withdraw<LPCoin2<X, Y>>(account, liquidity);
            (x_out, y_out) = remove_liquidity<X, Y>(coins, amount_x_min, amount_y_min);
        } else {
            let coins = coin::withdraw<LPCoin2<Y, X>>(account, liquidity);
    ...
    public entry fun withdraw_dao_fee<X, Y>(
        ...
        let amount = coin::balance<LPCoin2<X, Y>>(RESOURCE_ACCOUNT_ADDRESS) - MINIMUM_LIQUIDITY;
        coin::transfer<LPCoin2<X, Y>>(&get_resource_account_signer(), acc_addr, amount);
    }
    ...
    public fun create_pair<X, Y>() acquires AdminData, PairInfo {
        ...
        // create lp coin
        let (lp_b, lp_f, lp_m) = coin::initialize<LPCoin2<X, Y>>(&resource_account_signer, utf8(b"AnimeSwapLPCoin"), utf8(b"ANILPCoin"), 8, true);
        // register coin
        AnimeSwapPoolV1Library::register_coin<LPCoin2<X, Y>>(&resource_account_signer);
    ...
    public fun mint<X, Y>(
        coin_x: Coin<X>,
        coin_y: Coin<Y>
    ): Coin<LPCoin2<X, Y>> acquires LiquidityPool, AdminData, Events {
        assert!(AnimeSwapPoolV1Library::compare<X, Y>(), ERR_PAIR_ORDER_ERROR);
        assert!(exists<LiquidityPool<X, Y>>(RESOURCE_ACCOUNT_ADDRESS), ERR_PAIR_NOT_EXIST);
        assert_not_paused();
        ...
        let coins = coin::mint<LPCoin2<X, Y>>(liquidity, &lp.lp_mint_cap);
        // update interval
    ...
    /// Burn LPCoin2 and get back coins
    public fun burn<X, Y>(
        liquidity: Coin<LPCoin2<X, Y>>
    ): (Coin<X>, Coin<Y>) acquires LiquidityPool, AdminData, Events {
        assert!(AnimeSwapPoolV1Library::compare<X, Y>(), ERR_PAIR_ORDER_ERROR);
        assert_lp_unlocked<X, Y>();
        ...
        let (balance_x, balance_y) = (coin::value(&lp.coin_x_reserve), coin::value(&lp.coin_y_reserve));
        coin::burn<LPCoin2<X, Y>>(liquidity, &lp.lp_burn_cap);
    ...
    public entry fun test_add_remove_liquidity_basic_1_1(creator: &signer, resource_account: &signer, someone_else: &signer)
        ...
        // should takes 10000/10000 coin and gives 9000 LPCoin2 (AnimeSwapPoolV1Library::sqrt(10000*10000)-1000)
        add_liquidity_entry<BTC, USDT>(someone_else, 10000, 10000, 1, 1);
        {
            ...
            assert!(coin::balance<LPCoin2<BTC, USDT>>(signer::address_of(someone_else)) == 9000, USER_LP_BALANCE_ERROR);
        ...
        // should takes 100/100 coin and gives 100 LPCoin2
        add_liquidity_entry<BTC, USDT>(someone_else, 1000, 100, 1, 1);
        {
            ...
            assert!(coin::balance<LPCoin2<BTC, USDT>>(signer::address_of(someone_else)) == 9100, USER_LP_BALANCE_ERROR);
        ...
        // should takes 9000 LPCoin2 and gives 9000/9000 coin
        remove_liquidity_entry<BTC, USDT>(someone_else, 9000, 9000, 9000);
        {
            ...
            assert!(coin::balance<LPCoin2<BTC, USDT>>(signer::address_of(someone_else)) == 100, USER_LP_BALANCE_ERROR);
    ...
    public entry fun test_add_remove_liquidity_basic_1_100(creator: &signer, resource_account: &signer, someone_else: &signer)
        ...
        // should takes 1000/100000 coin and gives 9000 LPCoin2 (AnimeSwapPoolV1Library::sqrt(1000*100000)-1000)
        ...
        // should takes 10/1000 coin and gives 100 LPCoin2
        ...
        // should takes 9000 LPCoin2 and gives 900/90000 coin
    ...
    public entry fun test_add_remove_liquidity_basic_1_2(creator: &signer, resource_account: &signer, someone_else: &signer)
        ...
        // should takes 1000/2000 coin and gives 414 LPCoin2 (AnimeSwapPoolV1Library::sqrt(1000*2000)-1000)
        add_liquidity_entry<BTC, USDT>(someone_else, 1000, 2000, 1, 1);
        {
            let lp = borrow_global<LiquidityPool<BTC, USDT>>(RESOURCE_ACCOUNT_ADDRESS);
            assert!(coin::value(&lp.coin_x_reserve) == 1000, CONTRACTOR_BALANCE_ERROR);
            assert!(coin::value(&lp.coin_y_reserve) == 2000, CONTRACTOR_BALANCE_ERROR);
            assert!(coin::balance<LPCoin2<BTC, USDT>>(signer::address_of(someone_else)) == 414, USER_LP_BALANCE_ERROR);
        };

        // should takes 1000/2000 coin and gives 1414 LPCoin2
        add_liquidity_entry<BTC, USDT>(someone_else, 2000, 2000, 1, 1);
        {
            ...
            assert!(coin::balance<LPCoin2<BTC, USDT>>(signer::address_of(someone_else)) == 1828, USER_LP_BALANCE_ERROR);
        };

        // should takes 1828 LPCoin2 and gives 1828/2828*2000=1292|1828/2828*4000=2585 coin
        remove_liquidity_entry<BTC, USDT>(someone_else, 1828, 1, 1);
        {
            ...
            assert!(coin::balance<LPCoin2<BTC, USDT>>(signer::address_of(someone_else)) == 0, USER_LP_BALANCE_ERROR);
    ...
    public entry fun test_swap_basic_1(creator: &signer, resource_account: &signer, someone_else: &signer)
        ...
        // should takes 1000 LPCoin2 and gives 1000/10000*9914=991|1000/10000*10094=1009 coin
        remove_liquidity_entry<BTC, USDT>(someone_else, 1000, 1, 1);
        {
            ...
            assert!(coin::balance<LPCoin2<BTC, USDT>>(signer::address_of(someone_else)) == 8000, USER_LP_BALANCE_ERROR);
    ...
    public entry fun test_add_remove_liquidity_error_2(creator: &signer, resource_account: &signer, someone_else: &signer)
        ...
        // should takes 9000 LPCoin2 and gives 900/90000 coin, but expect more
    ...
    public entry fun test_add_multiple_liquidity(creator: &signer, resource_account: &signer, someone_else: &signer)
...

        {
            ...
            assert!(coin::balance<LPCoin2<USDT, Aptos>>(signer::address_of(someone_else)) == 99000, USER_LP_BALANCE_ERROR);
            assert!(coin::balance<LPCoin2<BTC, Aptos>>(signer::address_of(someone_else)) == 9000, USER_LP_BALANCE_ERROR);
            assert!(coin::balance<LPCoin2<BTC, USDT>>(signer::address_of(someone_else)) == 99000, USER_LP_BALANCE_ERROR);
        };
...

        {
            ...
            assert!(coin::balance<LPCoin2<USDT, Aptos>>(signer::address_of(someone_else)) == 199000, USER_LP_BALANCE_ERROR);
            assert!(coin::balance<LPCoin2<BTC, Aptos>>(signer::address_of(someone_else)) == 19000, USER_LP_BALANCE_ERROR);
            assert!(coin::balance<LPCoin2<BTC, USDT>>(signer::address_of(someone_else)) == 199000, USER_LP_BALANCE_ERROR);
        };
    ...
    public entry fun test_dao_fee(creator: &signer, resource_account: &signer, someone_else: &signer, dao_fee_to: &signer)
        ...
        add_liquidity_entry<BTC, USDT>(someone_else, 100000000, 100000000, 1, 1);
        {
            ...
            assert!(coin::balance<LPCoin2<BTC, USDT>>(signer::address_of(someone_else)) == 100000000 - 1000, USER_LP_BALANCE_ERROR);
        ...
        remove_liquidity_entry<BTC, USDT>(someone_else, 1000000, 1, 1);
        {
            ...
            assert!(coin::balance<LPCoin2<BTC, USDT>>(signer::address_of(someone_else)) == 98999000, USER_LP_BALANCE_ERROR);
            ...
            assert!(coin::balance<LPCoin2<BTC, USDT>>(RESOURCE_ACCOUNT_ADDRESS) == 1000 + 6819, USER_LP_BALANCE_ERROR);
        };

        // dao withdraw fee
        withdraw_dao_fee<BTC, USDT>(dao_fee_to);
        {
            assert!(coin::balance<LPCoin2<BTC, USDT>>(RESOURCE_ACCOUNT_ADDRESS) == 1000, USER_LP_BALANCE_ERROR);
            assert!(coin::balance<LPCoin2<BTC, USDT>>(signer::address_of(dao_fee_to)) == 6819, USER_LP_BALANCE_ERROR);
        };
    }

    // test resource account equal
    #[test(deployer = @SwapDeployer)]
    public entry fun test_resource2_account(deployer: &signer) {
        genesis::setup();
        create_account_for_test(signer::address_of(deployer));
        let addr = account::create_resource_address(&signer::address_of(deployer), x"31");
